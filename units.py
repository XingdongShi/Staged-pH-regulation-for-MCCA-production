...
class NAOHAdditionTank(MixTank):
    ...
    def _run(self):
        ...
        if hasattr(self, 'biomass_mass') and self.biomass_mass > 0:
            ...
            if len(self.ins) > 1:
                self.ins[1].imass['NaOH'] = required_naoh
...
class MCCAFermentation(StirredTankReactor):
    ...
    tau_cofermentation = 7.5 * 24 # hr; half of the 15 d total two-stage HRT
    ...
    def __init__(self, ID='', ins=None, outs=(), thermo=None, *, T=37+273.15,
                 ...
                 pH: float = 7.0,
                 hydrogen_yield: float = 0.03,  
                 methane_yield: float = 0.03,   
        ...
        self.P = P
        self.pH = float(pH)
        self.stage_role = 'neutral chain elongation'
        self.V_wf = V_wf
        ...
        self.tau = tau
        self.hydrogen_yield = float(hydrogen_yield)
        self.methane_yield = float(methane_yield)
    ...
    def _run(self):
        feed = self.ins[0]
        broth, gas, fermentation_sludge = self.outs
        ...
        gas.T = self.T
        gas.P = self.P
        gas.imass['H2'] = microalgae_mass * self.hydrogen_yield
        gas.imass['CH4'] = microalgae_mass * self.methane_yield
        # No independent sludge yield is specified for the black-box reactor.
        fermentation_sludge.empty()
    def _design(self):
        Design = self.design_results
    ...
    def set_hydrogen_yield(self, value: float):
        value = float(value)
        self.hydrogen_yield = value
...
@cost(basis='Fermenter size', ID='Fermenter agitator', units='kg',
      ...
      S=(42607+443391+948+116)*(60+36),
      CE=CEPCI[2009], n=0.7, BM=1.5)
class MCCAFermentation_acidic(StirredTankReactor):
    _N_ins = 1
    _N_outs = 3
    ...
    def __init__(self, ID='', ins=None, outs=(), thermo=None, *, T=37+273.15,
                 ...
                 microalgae_mass_flow: float | None = None,  # kg/hr basis of original algae for yield calc
                 pH: float = 4.5,
                 hydrogen_yield: float = 0.05, 
                 methane_yield: float = 0.05,   
                 # allow_dilution=False,
                 # allow_concentration=False,
                 # sugars=None
                 ):
        ...
        self.P = P
        self.pH = float(pH)
        self.stage_role = 'acidic ethanol-producing fermentation'
        self.V_wf = V_wf
        self.length_to_diameter = length_to_diameter
        self.mixing_intensity = mixing_intensity
        self.kW_per_m3 = kW_per_m3
...
@cost(basis='Fermenter size', ID='Fermenter agitator', units='kg',
      ...
      S=(42607+443391+948+116)*(60+36),
      CE=CEPCI[2009], n=0.7, BM=1.5)
class MCCAFermentation_control(StirredTankReactor):
    _N_ins = 1
    _N_outs = 3
    ...
    def __init__(self, ID='', ins=None, outs=(), thermo=None, *, T=37+273.15,
                 ...
                 microalgae_mass_flow: float | None = None,  # kg/hr basis of original algae for yield calc
                 pH: float = 7.0,
                 hydrogen_yield: float = 0.03,  # kg H2 per kg dry microalgae
                 methane_yield: float = 0.03,   # kg CH4 per kg dry microalgae
                 # allow_dilution=False,
                 # allow_concentration=False,
                 # sugars=None
                 ):
        ...
        self.P = P
        self.pH = float(pH)
        self.stage_role = 'single-stage pH 7 control fermentation'
        self.V_wf = V_wf
        self.length_to_diameter = length_to_diameter
        self.mixing_intensity = mixing_intensity
        self.kW_per_m3 = kW_per_m3
    ...
    def set_hydrogen_yield(self, value: float):
        value = float(value)
        if value < 0:
            raise ValueError('Hydrogen yield must be nonnegative.')
        self.hydrogen_yield = value


class MCCAFermentation_ethanol(MCCAFermentation):
    """Single-stage empirical MCCA fermentation with external ethanol.

    Product yields and hydrogen generation follow the Bioindustrial-Park
    microalgae ethanol-fed reference case and are expressed per kg dry
    microalgae. The supplied ethanol is a required external electron donor.
    """

    _N_ins = 2  # hydrolysate, external ethanol
    _N_outs = 3  # broth, fermentation gas, sludge
    ...
    def __init__(self, ID='', ins=None, outs=(), thermo=None, *,
                 hydrogen_yield: float = 0.03,
                 methane_yield: float = 0.03, **kwargs):
        super().__init__(ID, ins, outs, thermo, hydrogen_yield=hydrogen_yield,
                         methane_yield=methane_yield, **kwargs)
        self.stage_role = 'external-ethanol chain elongation'
        # The empirical fermentation is a black box parameterized by measured
        # product yields. External ethanol is therefore treated as a consumed
        # electron donor rather than an unreacted stream available for energy
        # recovery downstream.
        self.ethanol_utilization = 1.0

    def _run(self):
        hydrolysate, external_ethanol = self.ins
        broth, gas, fermentation_sludge = self.outs
        broth.mix_from((hydrolysate, external_ethanol))
        broth.imass['Ethanol'] *= 1.0 - self.ethanol_utilization
        broth.T = self.T
        broth.P = self.P
        microalgae_mass = self.microalgae_mass_flow
        base_yields = self.reference_yields
        base_c6 = base_yields['CaproicAcid']
        c6_yield = getattr(self, 'caproic_acid_yield_absolute', None)
        c6_yield = base_c6 * getattr(self, 'caproic_acid_yield_factor', 1.0) \
            if c6_yield is None else float(c6_yield)
        adjustment = max(
            0.1,
            (sum(base_yields.values()) - c6_yield)
            / (sum(base_yields.values()) - base_c6),
        )
        for chemical, value in base_yields.items():
            broth.imass[chemical] = microalgae_mass * (
                c6_yield if chemical == 'CaproicAcid' else value * adjustment
            )
        gas.empty()
        gas.phase = 'g'
        gas.T = self.T
...
class SolidLiquidSeparation(Unit):
    ...
    def _run(self):
        ...
        residue.empty()
        insolubles = ('Protein', 'Lipid', 'Ash', 'TriOlein', 'AlphaAmylase', 'GlucoAmylase')
...
class AnaerobicDigestion(bst.Unit):
    ...
    def __init__(self, ID='', ins=None, outs=(), *, tau=15*24,
                 hydrogen_yield=0.03, methane_yield=0.03,
                 design_microalgae_mass: float, **kwargs):
        super().__init__(ID, ins, outs, **kwargs)
        self.tau = tau
        self.hydrogen_yield = float(hydrogen_yield)
        self.methane_yield = float(methane_yield)
        self.design_microalgae_mass = design_microalgae_mass
        self.T = 37 + 273.15
    ...
    def _run(self):
        ...
        biogas.empty()
        biogas.phase = 'g'
        biogas.imass['H2'] = self.design_microalgae_mass * self.hydrogen_yield
        biogas.imass['CH4'] = self.design_microalgae_mass * self.methane_yield
        biogas.T = self.T
        waste.empty()
        sludge.empty()
        for ID in self.degrade_IDs:
            amount = feed.imass[ID]
            waste.imass[ID] = amount * 0.15 # 15% into wastewater
            sludge.imass[ID] = amount * 0.85 # 85% into sludge
    ...
    def _design(self):
        ...
        # Screening heat-duty assumption, scaled with dry-microalgae design
        # throughput. This is not a heat balance derived from the empirical
        # H2/CH4 generation or from the residual-stream composition.
        duty = 800 * self.design_microalgae_mass  # Heat duty
        self.add_heat_utility(duty, self.T)
