...
# Capital costs are escalated to 2016 USD. Operating prices are shared by the
# two-stage and control scenarios and are treated as common scenario inputs;
# no source-year conversion is implied unless documented for that item.
CAPITAL_COST_BASIS_YEAR = 2016

# Price dictionary for all relevant feedstocks, utilities, chemicals, and products
price = {
    'Microalgae': 0.15,  # $/kg dry microalgae; cultivation, harvesting, and transport
    'NaturalGas': 0.2527,  # $/kg
    ...
    # Source reports 1.00 $/lb; convert before assigning the BioSTEAM $/kg price.
    'OleylAlcohol': 1.00 * _lb_per_kg,  # $/kg
    'AceticAcid': 1.31, # $/kg https://catcost.chemcatbio.org/materials-library
    'PropionicAcid': 1.50, # $/kg assumed for product valuation
...
# Lock capital-cost escalation and the shared electricity assumption instead
# of relying on BioSTEAM's version-dependent defaults.
bst.CE = CEPCI[CAPITAL_COST_BASIS_YEAR]
